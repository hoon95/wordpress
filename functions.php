...
if(! function_exists('minimal_paginate_links')){

  function minimal_paginate_links() {
      ...
      for ( $n = 1; $n <= $total; $n++ ) :
        if ( $n == $current ) :
          $page_links[] = sprintf(
            '<span aria-current="%s" class="secondary-btn active">%s</span>',
          ...
          if ( $args['show_all'] || ( $n <= $end_size || ( $current && $n >= $current - $mid_size && $n <= $current + $mid_size ) || $n > $total - $end_size ) ) :
            ...
            $page_links[] = sprintf(
              '<a class="secondary-btn" href="%s">%s</a>',
    ...
    //     echo '<a href="" class="secondary-btn">'.$page.'</a>';
    //   }
    // //}
